...
module Project2 =
    ...
    let meanSquaredError (x:(float*float) seq) = //This implements MSE loss which we found was accurate enough for our analyses for regression problems
        x                                        //take the tuples
        |>Seq.averageBy (fun (a,b) -> System.Math.Abs(a-b))  //and average them all by squaring the difference of their parts
    ////
    ...
    let createKEditedNearestNeighbor k (trainingSet:Point seq) =
        trainingSet                                 // take the trainingSet
        |> Seq.mapi ( fun i point ->                // take a point and the index of said point
            point,(kNearestNeighborClassification k (     // we are making a kNNclassifier
                trainingSet                         // using the trainingSet                        //we are taking out the point we are looking at...
                |> Seq.filterWithIndex (fun j _ -> j<>i)   // make sure the index you are looking at is not our input point
                )).classify point                    // then we classify the point in question
            ) 
        |> Seq.filterWithIndex (fun i (_,x) ->                 // take the tuple...
            x <> (trainingSet|>Seq.toArray).[i].cls         // filter out points that are falsely classified
        )
        |>Seq.map fst
    ...
    let kMeansImpl k (trainingSet:Point seq) =
        ...
        let pointsMidpoint (ps :Point seq) =                                                        // get the mean of the points in a sequence
            ...
            // assign the class value, regression value, real and categorical attributes to the point object that represents the midpoints of ps
            {
                cls = maxClassValue
        ...
        let getCentroids (centroids:Point seq) (dataset:Point seq) =    //we are taking our centroids and our dataset
            let epsilon = 1.                                            //here we are accepting an error of upto 100% (much lower and timecomplexity gets really bad.
            let attributeDetails = createAttributeDetails dataset       //get the attribute details for the dataset (this allows us to get the distance between points quicker)
            let rec loop (centroids':Point seq) deltaDist =             //recursively loop over the centroids keeping track of the distance value obtained in the last loop
                ...
                |>Seq.map (fun (m,ps) ->                                                                    //we take the midpoint and the points associated with it
                    m,(ps |> pointsMidpoint),ps                                                             //and we obtain a new midpoint making: (oldmidpoint*newmidpoint*points)
                )
                |> (fun centroids'' ->                                                                      //now we take the three tuple
                    let deltaDist' =  centroids''|>Seq.sumBy(fun (oldCentroid,newCentroid,_)-> (oldCentroid.distance newCentroid attributeDetails)) //we set the new distance from the old to new centroids
                    match ((System.Math.Abs (deltaDist'-deltaDist)/deltaDist)) with                         //we match the change in deltaDist from the last run to this run.
                    | v when v > epsilon -> loop (centroids'' |> Seq.map (fun (_,b,_) -> b)) deltaDist'     //if the change in dist is greater or euqal to epsilon, we loop with the new centroids being the centroids, and deltaDist' becoming the "old" dist
                    | _ -> (centroids'' |> Seq.map (fun (_,b,c) -> b,c))                                    //otherwise, return the current centroids and thier clusters
                )
            loop centroids 1.                                                                               //we then call loop with a small initial distance that will be overwritten immedietly
        
        getCentroids firstCentroids trainingSet //Get the K means...
        
    
    let kMedoids k (trainingSet:Point seq) =                        //take k and our points
        let attributeDetails = createAttributeDetails trainingSet   //get the attribute details on our training set
        let distortion (point:Point) (targets:Point seq) =          //this function will get the distortion from point to all targets.
            targets                                                 //take the sequence of targets 
            |> Seq.sumBy(fun p -> point.distance p attributeDetails)//and return the summed distances to each point.
         
        kMeansImpl k trainingSet                                    //run kMeans on k and our training set
        |> Seq.map (fun (m,ps)->                                    //take the resulting (point*point[]) seq and map to...
            seq {                                                   //create a new sequence
                yield (m,distortion m ps)                           //the first element is our centroid ad it's distorition
                yield! ps|>Seq.map(fun p -> (p,distortion p ps))    //then add each element in the cluster with it's distorition
            }
            |>Seq.minBy snd                                         //get the minimum tuple by the distorition
            |>fst                                                   //return the resulting centroid
        )
...
//Get the Datasets (from chris's computer
let ds1 = (fullDataset @"C:\Users\chris\OneDrive\Documents\CSCI447\MachineLearningProject2\Data\abalone.data" (Some 0) None 2. true false) //filename classIndex regressionIndex pValue isCommaSeperated hasHeader
let ds2 = (fullDataset @"C:\Users\chris\OneDrive\Documents\CSCI447\MachineLearningProject2\Data\car.data" (Some 6) None 2. true false)
let ds3 = (fullDataset @"C:\Users\chris\OneDrive\Documents\CSCI447\MachineLearningProject2\Data\forestfires.csv" None (Some 12) 2. true true)
let ds4 = (fullDataset @"C:\Users\chris\OneDrive\Documents\CSCI447\MachineLearningProject2\Data\machine.data" None (Some 9) 2. true false )
let ds5 = (fullDataset @"C:\Users\chris\OneDrive\Documents\CSCI447\MachineLearningProject2\Data\segmentation.data" (Some 0) None 2. true true)
let ds6 = (fullDataset @"C:\Users\chris\OneDrive\Documents\CSCI447\MachineLearningProject2\Data\winequality-red.csv" None (Some 11) 2. false true)
let ds7 = (fullDataset @"C:\Users\chris\OneDrive\Documents\CSCI447\MachineLearningProject2\Data\winequality-white.csv" None (Some 11) 2. false true)
let ds67 = (fullDataset @"C:\Users\chris\OneDrive\Documents\CSCI447\MachineLearningProject2\Data\reduced_winedata.data" None (Some 11) 2. true true)
...
let applyKFold (trainingSet:Point seq) (validationSet: Point seq) (preprocessFunction: (Point seq -> Point seq) option) (kfoldType) =   //apply the loss function (MSE) to the kth fold
    
    let workingTrainingSet = (match preprocessFunction with |Some f -> (f trainingSet) |_ -> trainingSet)//preprocess our validation set if we have a preprocessing function, otherwise use our validation set
...
let doKFold k (dataSet:Point seq) (preprocessFunction: (Point seq -> Point seq) option) kfoldType =           //This is where we do the k-folding algorithim this will return the average from all the kfolds
...
let res = 
    ...
    |>Seq.map (fun k' ->
        ...
        |> Seq.map (fun (ds,(knnc,knnr,enn,cnn)) -> ds,(knnc,knnr,enn,cnn,(if ds.[1].cls.IsSome then printf "\tkm-knnc"; doKFold 10 ds (Some (fun d -> createKEditedNearestNeighbor k' d|> kMeansImpl k' |>Seq.map fst )) (Classification((kNearestNeighborClassification 1),zeroOneLoss)) else 0.0)))
        |> Seq.map (fun (ds,(knnc,knnr,enn,cnn,kmc)) -> ds,(knnc,knnr,enn,cnn,kmc,(if ds.[1].cls.IsNone then printf "\tkm-knnr"; doKFold 10 ds (Some (fun d -> d|>Seq.take ((d|>Seq.length)/4)|> kMeansImpl k' |>Seq.map fst )) (Regression((kNearestNeighborRegression 1),meanSquaredError)) else 0.0)))
        |> Seq.map (fun (ds,(knnc,knnr,enn,cnn,kmc,kmr)) -> ds,(knnc,knnr,enn,cnn,kmc,kmr,(if ds.[1].cls.IsSome then printf "\tpam-knnc"; doKFold 10 ds (Some (fun d -> createKEditedNearestNeighbor k' d|> kMedoids k')) (Classification((kNearestNeighborClassification 1),zeroOneLoss)) else 0.0)))
        |> Seq.map (fun (ds,(knnc,knnr,enn,cnn,kmc,kmr,pamc)) -> ds,(knnc,knnr,enn,cnn,kmc,kmr,pamc,(if ds.[1].cls.IsNone then printf "\tpam-knnr"; doKFold 10 ds (Some (fun d -> d|>Seq.take ((d|>Seq.length)/4)|> kMedoids k')) (Regression((kNearestNeighborRegression 1),meanSquaredError)) else 0.0)))
        |> Seq.map (fun (_,(knnc,knnr,enn,cnn,kmc,kmr,pamc,pamr)) ->(knnc,knnr,enn,cnn,kmc,kmr,pamc,pamr))
        |>Seq.toArray
        |>Seq.iteri (fun i (knnc,knnr,enn,cnn,kmc,kmr,pamc,pamr) -> (printfn "ds%d: knnc: %f knnr: %f enn: %f cnn:%f kmc:%f kmr: %f pamc:%f pamr:%f" i knnc knnr enn cnn kmc kmr pamc pamr) )
    )
    |>Seq.toList
//sw.Stop()
